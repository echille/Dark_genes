...
```{r}
#Set DESeq2 design
gdds_dev <- DESeqDataSetFromMatrix(countData = round(gcount_filt, digits = 0),
...
```{r}
gPCAdata_dev <- plotPCA(gvst_dev, intgroup = c("BioProject", "Organism", "Life_Stage"), returnData=TRUE)
percentVar_dev <- round(100*attr(gPCAdata_dev, "percentVar")) #plot PCA of samples with all data

allgenesfilt_PCA <- ggplot(gPCAdata_dev, aes(PC1, PC2, colour=Life_Stage)) + 
  geom_point(size=0.5) +
